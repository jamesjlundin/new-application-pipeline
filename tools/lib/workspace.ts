...
export function getRepoContext(
  ...
  options: { focusPatterns?: string[]; maxFileSize?: number } = {}
): string {
  const { focusPatterns = [], maxFileSize = 200 } = options;
  const sections: string[] = [];

  // Always include tree
  sections.push('## Repository Structure\n');
  sections.push('```');
  sections.push(getFileTree(workspacePath, 3));
  sections.push('```\n');

  // Include key configs (condensed)
  const configs = getKeyConfigFiles(workspacePath);
  const priorityConfigs = ['package.json', 'tsconfig.json', 'turbo.json', 'pnpm-workspace.yaml'];
  for (const key of priorityConfigs) {
    if (configs[key]) {
      const ext = path.extname(key).slice(1) || 'text';
      sections.push(`### ${key}\n`);
      sections.push(`\`\`\`${ext}`);
      sections.push(configs[key]);
      sections.push('```\n');
    }
  }

  // If focus patterns provided, grep for relevant files
  if (focusPatterns.length > 0) {
    sections.push('## Relevant Code Excerpts\n');
    for (const pattern of focusPatterns) {
      try {
        const result = execSync(
          `grep -rl "${pattern}" --include="*.ts" --include="*.tsx" --include="*.js" --include="*.jsx" . | head -10`,
          {
            cwd: workspacePath,
            encoding: 'utf-8',
            maxBuffer: 5 * 1024 * 1024,
        ...
        for (const file of files) {
          const content = readFileIfExists(path.join(workspacePath, file), maxFileSize);
          if (content) {
            sections.push(`### ${file} (matches: "${pattern}")\n`);
